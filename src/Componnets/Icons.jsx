export const Icons = {
  call: (props) => (
    <svg
      viewBox="0 0 24 24"
      fill="current Color"
      xmlns="http://www.w3.org/2000/svg"
      {...props}>
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"></g>
      <g id="SVGRepo_iconCarrier">
        <path
          fillRule="evenodd"
          clipRule="evenodd"
          d="M6.00745 3.40686C7.68752 1.72679 10.5227 1.85451 11.6925 3.95063L12.3415 5.11356C13.1054 6.48238 12.7799 8.20946 11.6616 9.34143C11.6467 9.36184 11.5677 9.47677 11.5579 9.67758C11.5454 9.93391 11.6364 10.5267 12.5548 11.4451C13.4729 12.3632 14.0656 12.4545 14.3221 12.442C14.5231 12.4322 14.6381 12.3533 14.6585 12.3383C15.7905 11.2201 17.5176 10.8945 18.8864 11.6584L20.0493 12.3075C22.1454 13.4773 22.2731 16.3124 20.5931 17.9925C19.6944 18.8911 18.4995 19.6896 17.0953 19.7429C15.0144 19.8218 11.5591 19.2844 8.13735 15.8626C4.71556 12.4408 4.17818 8.98556 4.25706 6.90463C4.3103 5.50044 5.10879 4.30552 6.00745 3.40686ZM10.3827 4.68163C9.78363 3.60828 8.17394 3.36169 7.06811 4.46752C6.29276 5.24287 5.7887 6.09868 5.75599 6.96146C5.6902 8.6968 6.11864 11.7226 9.19801 14.8019C12.2774 17.8813 15.3031 18.3097 17.0385 18.2439C17.9013 18.2112 18.7571 17.7072 19.5324 16.9318C20.6382 15.826 20.3916 14.2163 19.3183 13.6173L18.1554 12.9683C17.432 12.5645 16.4158 12.7023 15.7025 13.4156L15.7023 13.4158C15.6322 13.4858 15.1864 13.9018 14.395 13.9403C13.5847 13.9797 12.604 13.6156 11.4942 12.5058C10.384 11.3956 10.02 10.4146 10.0597 9.60423C10.0985 8.81271 10.5147 8.36711 10.5843 8.29746L10.5844 8.29743C11.2977 7.58411 11.4354 6.56797 11.0317 5.84456L10.3827 4.68163Z"></path>
      </g>
    </svg>
  ),
  bag: (props) => (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}>
      <g id="SVGRepo_bgCarrier" strokeWidth="2"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"></g>
      <g id="SVGRepo_iconCarrier">
        <path
          d="M3.79424 12.0291C4.33141 9.34329 4.59999 8.00036 5.48746 7.13543C5.65149 6.97557 5.82894 6.8301 6.01786 6.70061C7.04004 6 8.40956 6 11.1486 6H12.8515C15.5906 6 16.9601 6 17.9823 6.70061C18.1712 6.8301 18.3486 6.97557 18.5127 7.13543C19.4001 8.00036 19.6687 9.34329 20.2059 12.0291C20.9771 15.8851 21.3627 17.8131 20.475 19.1793C20.3143 19.4267 20.1267 19.6555 19.9157 19.8616C18.7501 21 16.7839 21 12.8515 21H11.1486C7.21622 21 5.25004 21 4.08447 19.8616C3.87342 19.6555 3.68582 19.4267 3.5251 19.1793C2.63744 17.8131 3.02304 15.8851 3.79424 12.0291Z"
          strokeWidth="2.5"></path>
        <circle cx="15" cy="9" r="1" fill="#A2A2A2"></circle>
        <circle cx="9" cy="9" r="1" fill="#A2A2A2"></circle>
        <path
          d="M9 6V5C9 3.34315 10.3431 2 12 2C13.6569 2 15 3.34315 15 5V6"
          strokeWidth="1.5"
          strokeLinecap="round"></path>
      </g>
    </svg>
  ),
  favorite: (props) => (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}>
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"></g>
      <g id="SVGRepo_iconCarrier">
        <path d="M8.96173 18.9109L9.42605 18.3219L8.96173 18.9109ZM12 5.50063L11.4596 6.02073C11.601 6.16763 11.7961 6.25063 12 6.25063C12.2039 6.25063 12.399 6.16763 12.5404 6.02073L12 5.50063ZM15.0383 18.9109L15.5026 19.4999L15.0383 18.9109ZM9.42605 18.3219C7.91039 17.1271 6.25307 15.9603 4.93829 14.4798C3.64922 13.0282 2.75 11.3345 2.75 9.1371H1.25C1.25 11.8026 2.3605 13.8361 3.81672 15.4758C5.24723 17.0866 7.07077 18.3752 8.49742 19.4999L9.42605 18.3219ZM2.75 9.1371C2.75 6.98623 3.96537 5.18252 5.62436 4.42419C7.23607 3.68748 9.40166 3.88258 11.4596 6.02073L12.5404 4.98053C10.0985 2.44352 7.26409 2.02539 5.00076 3.05996C2.78471 4.07292 1.25 6.42503 1.25 9.1371H2.75ZM8.49742 19.4999C9.00965 19.9037 9.55954 20.3343 10.1168 20.6599C10.6739 20.9854 11.3096 21.25 12 21.25V19.75C11.6904 19.75 11.3261 19.6293 10.8736 19.3648C10.4213 19.1005 9.95208 18.7366 9.42605 18.3219L8.49742 19.4999ZM15.5026 19.4999C16.9292 18.3752 18.7528 17.0866 20.1833 15.4758C21.6395 13.8361 22.75 11.8026 22.75 9.1371H21.25C21.25 11.3345 20.3508 13.0282 19.0617 14.4798C17.7469 15.9603 16.0896 17.1271 14.574 18.3219L15.5026 19.4999ZM22.75 9.1371C22.75 6.42503 21.2153 4.07292 18.9992 3.05996C16.7359 2.02539 13.9015 2.44352 11.4596 4.98053L12.5404 6.02073C14.5983 3.88258 16.7639 3.68748 18.3756 4.42419C20.0346 5.18252 21.25 6.98623 21.25 9.1371H22.75ZM14.574 18.3219C14.0479 18.7366 13.5787 19.1005 13.1264 19.3648C12.6739 19.6293 12.3096 19.75 12 19.75V21.25C12.6904 21.25 13.3261 20.9854 13.8832 20.6599C14.4405 20.3343 14.9903 19.9037 15.5026 19.4999L14.574 18.3219Z"></path>
      </g>
    </svg>
  ),
  home: (props) => (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}>
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"></g>
      <g id="SVGRepo_iconCarrier">
        <path
          fillRule="evenodd"
          clipRule="evenodd"
          d="M12.2796 3.71579C12.097 3.66261 11.903 3.66261 11.7203 3.71579C11.6678 3.7311 11.5754 3.7694 11.3789 3.91817C11.1723 4.07463 10.9193 4.29855 10.5251 4.64896L5.28544 9.3064C4.64309 9.87739 4.46099 10.0496 4.33439 10.24C4.21261 10.4232 4.12189 10.6252 4.06588 10.8379C4.00765 11.0591 3.99995 11.3095 3.99995 12.169V17.17C3.99995 18.041 4.00076 18.6331 4.03874 19.0905C4.07573 19.536 4.14275 19.7634 4.22513 19.9219C4.41488 20.2872 4.71272 20.5851 5.07801 20.7748C5.23658 20.8572 5.46397 20.9242 5.90941 20.9612C6.36681 20.9992 6.95893 21 7.82995 21H7.99995V18C7.99995 15.7909 9.79081 14 12 14C14.2091 14 16 15.7909 16 18V21H16.17C17.041 21 17.6331 20.9992 18.0905 20.9612C18.5359 20.9242 18.7633 20.8572 18.9219 20.7748C19.2872 20.5851 19.585 20.2872 19.7748 19.9219C19.8572 19.7634 19.9242 19.536 19.9612 19.0905C19.9991 18.6331 20 18.041 20 17.17V12.169C20 11.3095 19.9923 11.0591 19.934 10.8379C19.878 10.6252 19.7873 10.4232 19.6655 10.24C19.5389 10.0496 19.3568 9.87739 18.7145 9.3064L13.4748 4.64896C13.0806 4.29855 12.8276 4.07463 12.621 3.91817C12.4245 3.7694 12.3321 3.7311 12.2796 3.71579ZM11.1611 1.79556C11.709 1.63602 12.2909 1.63602 12.8388 1.79556C13.2189 1.90627 13.5341 2.10095 13.8282 2.32363C14.1052 2.53335 14.4172 2.81064 14.7764 3.12995L20.0432 7.81159C20.0716 7.83679 20.0995 7.86165 20.1272 7.88619C20.6489 8.34941 21.0429 8.69935 21.3311 9.13277C21.5746 9.49916 21.7561 9.90321 21.8681 10.3287C22.0006 10.832 22.0004 11.359 22 12.0566C22 12.0936 22 12.131 22 12.169V17.212C22 18.0305 22 18.7061 21.9543 19.2561C21.9069 19.8274 21.805 20.3523 21.5496 20.8439C21.1701 21.5745 20.5744 22.1701 19.8439 22.5496C19.3522 22.805 18.8274 22.9069 18.256 22.9543C17.706 23 17.0305 23 16.2119 23H15.805C15.7972 23 15.7894 23 15.7814 23C15.6603 23 15.5157 23.0001 15.3883 22.9895C15.2406 22.9773 15.0292 22.9458 14.8085 22.8311C14.5345 22.6888 14.3111 22.4654 14.1688 22.1915C14.0542 21.9707 14.0227 21.7593 14.0104 21.6116C13.9998 21.4843 13.9999 21.3396 13.9999 21.2185L14 18C14 16.8954 13.1045 16 12 16C10.8954 16 9.99995 16.8954 9.99995 18L9.99996 21.2185C10 21.3396 10.0001 21.4843 9.98949 21.6116C9.97722 21.7593 9.94572 21.9707 9.83107 22.1915C9.68876 22.4654 9.46538 22.6888 9.19142 22.8311C8.9707 22.9458 8.75929 22.9773 8.6116 22.9895C8.48423 23.0001 8.33959 23 8.21847 23C8.21053 23 8.20268 23 8.19495 23H7.78798C6.96944 23 6.29389 23 5.74388 22.9543C5.17253 22.9069 4.64769 22.805 4.15605 22.5496C3.42548 22.1701 2.8298 21.5745 2.4503 20.8439C2.19492 20.3523 2.09305 19.8274 2.0456 19.2561C1.99993 18.7061 1.99994 18.0305 1.99995 17.212L1.99995 12.169C1.99995 12.131 1.99993 12.0936 1.99992 12.0566C1.99955 11.359 1.99928 10.832 2.1318 10.3287C2.24383 9.90321 2.42528 9.49916 2.66884 9.13277C2.95696 8.69935 3.35105 8.34941 3.87272 7.8862C3.90036 7.86165 3.92835 7.83679 3.95671 7.81159L9.22354 3.12996C9.58274 2.81064 9.89467 2.53335 10.1717 2.32363C10.4658 2.10095 10.781 1.90627 11.1611 1.79556Z"></path>
      </g>
    </svg>
  ),
  instagram: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="32"
      height="32"
      viewBox="0 0 24 24"
      fill="none"
      stroke="current Color"
      className="w-8"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
      {...props}>
      <rect width="20" height="20" x="2" y="2" rx="5" ry="5" />
      <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z" />
      <line x1="17.5" x2="17.51" y1="6.5" y2="6.5" />
    </svg>
  ),
  location: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      fill="none"
      viewBox="0 0 24 24"
      strokeWidth={1.5}
      stroke="current Color"
      {...props}>
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"
      />
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1 1 15 0Z"
      />
    </svg>
  ),
  bell: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      fill="none"
      viewBox="0 0 24 24"
      strokeWidth={1.5}
      stroke="current Color"
      {...props}>
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M14.857 17.082a23.848 23.848 0 0 0 5.454-1.31A8.967 8.967 0 0 1 18 9.75V9A6 6 0 0 0 6 9v.75a8.967 8.967 0 0 1-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 0 1-5.714 0m5.714 0a3 3 0 1 1-5.714 0M3.124 7.5A8.969 8.969 0 0 1 5.292 3m13.416 0a8.969 8.969 0 0 1 2.168 4.5"
      />
    </svg>
  ),
  wallet: (props) => (
    <svg
      viewBox="0 0 19 17"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}>
      <path
        d="M17.533 10.497h-3.374a2.244 2.244 0 0 1 0-4.485h3.374m-2.993 2.19h-.259"
        stroke="current Color"
        strokeWidth={1.5}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        clipRule="evenodd"
        d="M5.957 1h7.202a4.373 4.373 0 0 1 4.373 4.373v5.98a4.373 4.373 0 0 1-4.373 4.374H5.957a4.373 4.373 0 0 1-4.373-4.373v-5.98A4.373 4.373 0 0 1 5.957 1"
        stroke="current Color"
        strokeWidth={1.5}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M5.363 4.782h4.5"
        stroke="current Color"
        strokeWidth={1.5}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  ),
  table: (props) => (
    <svg
      viewBox="0 0 14 13"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}>
      <path
        d="M14 3.31377C14 1.53604 10.3929 0.575756 7.00152 0.575756C3.61016 0.575756 0 1.53604 0 3.31377C0 5.03681 3.38225 5.991 6.6794 6.04875V9.63156C6.67333 9.65284 6.66421 9.67107 6.65509 9.69232C6.52441 9.70144 6.412 9.79261 6.3816 9.92329C6.34818 10.0418 6.3026 10.1542 6.24485 10.2636C6.21445 10.2788 6.18103 10.2879 6.14762 10.294C5.79512 10.3396 5.29066 9.78959 5.10226 9.51609C5.00804 9.37629 4.8166 9.34287 4.67985 9.43709C4.5431 9.5313 4.50663 9.72275 4.60085 9.8595C4.66771 9.95673 5.18126 10.6861 5.82552 10.8623C5.45478 11.2634 4.85611 11.6706 3.8928 11.8196C3.72566 11.8439 3.61324 11.9988 3.63754 12.166C3.65882 12.3149 3.78947 12.4243 3.93839 12.4243C3.95357 12.4243 3.96878 12.4243 3.98397 12.4213C5.60367 12.1751 6.36946 11.2908 6.71287 10.6769C6.8162 10.5888 6.90737 10.4885 6.9803 10.373C7.05323 10.4855 7.1444 10.5888 7.24773 10.6769C7.59416 11.2908 8.35995 12.1751 9.97965 12.4213C9.99484 12.4243 10.01 12.4243 10.0252 12.4243C10.1924 12.4243 10.3291 12.2876 10.3291 12.1204C10.3291 11.9685 10.2197 11.8408 10.0708 11.8196C9.10446 11.6706 8.50277 11.2634 8.13506 10.8623C8.77928 10.683 9.29591 9.95371 9.36274 9.85645C9.45696 9.71665 9.42049 9.52825 9.28374 9.43404C9.14395 9.33983 8.95555 9.37629 8.86133 9.51304C8.67595 9.78654 8.17152 10.3396 7.81598 10.291C7.78256 10.2879 7.74609 10.2758 7.71569 10.2606C7.66099 10.1512 7.61541 10.0388 7.58199 9.92024C7.5516 9.79261 7.43918 9.69839 7.3085 9.69232C7.29938 9.67104 7.29331 9.65281 7.28419 9.63156L7.28724 6.05179C10.5935 6.00011 14 5.04287 14 3.31377ZM7.07446 5.44705C7.02583 5.42577 6.96811 5.42577 6.91948 5.44705C3.21208 5.42577 0.632092 4.3166 0.632092 3.31984C0.632092 2.31397 3.2516 1.19263 7.01369 1.19263C10.7758 1.19263 13.3953 2.31397 13.3953 3.31984C13.3953 4.31962 10.8031 5.43489 7.07446 5.44705Z"
        fill="current Color"
        stroke="current Color"
        strokeWidth={0.5}
      />
    </svg>
  ),
  plus: (props) => (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}>
      <g strokeWidth={0} />
      <g strokeLinecap="round" strokeLinejoin="round" />
      <path
        d="M5 12h14m-7-7v14"
        strokeWidth={2}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  ),
  mines: (props) => (
    <svg
      viewBox="0 0 20 20"
      xmlns="http://www.w3.org/2000/svg"
      fill="currentColor"
      {...props}>
      <g strokeWidth={0} />
      <g strokeLinecap="round" strokeLinejoin="round" />
      <path
        fillRule="evenodd"
        d="M18 10a1 1 0 0 1-1 1H3a1 1 0 1 1 0-2h14a1 1 0 0 1 1 1"
      />
    </svg>
  ),
  search: (props) => (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}>
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"></g>
      <g id="SVGRepo_iconCarrier">
        <path
          d="M15.7955 15.8111L21 21M18 10.5C18 14.6421 14.6421 18 10.5 18C6.35786 18 3 14.6421 3 10.5C3 6.35786 6.35786 3 10.5 3C14.6421 3 18 6.35786 18 10.5Z"
          stroke="current Color"
          strokeWidth="2"
          strokeLinecap="round"
          strokeLinejoin="round"></path>
      </g>
    </svg>
  ),
  darkMode: (props) => (
    <svg viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg" {...props}>
      <g id="SVGRepo_bgCarrier" strokeWidth={0} />
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <g id="SVGRepo_iconCarrier">
        <title>{"dark-mode"}</title>
        <g id="Layer_2" data-name="Layer 2">
          <g id="Icons">
            <g>
              <rect width={48} height={48} fill="none" />
              <g>
                <path d="M14,24A10,10,0,0,0,24,34V14A10,10,0,0,0,14,24Z" />
                <path d="M24,2A22,22,0,1,0,46,24,21.9,21.9,0,0,0,24,2ZM6,24A18.1,18.1,0,0,1,24,6v8a10,10,0,0,1,0,20v8A18.1,18.1,0,0,1,6,24Z" />
              </g>
            </g>
          </g>
        </g>
      </g>
    </svg>
  ),
  like: (props) => (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}>
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"></g>
      <g id="SVGRepo_iconCarrier">
        <path d="M8.96173 18.9109L9.42605 18.3219L8.96173 18.9109ZM12 5.50063L11.4596 6.02073C11.601 6.16763 11.7961 6.25063 12 6.25063C12.2039 6.25063 12.399 6.16763 12.5404 6.02073L12 5.50063ZM15.0383 18.9109L15.5026 19.4999L15.0383 18.9109ZM9.42605 18.3219C7.91039 17.1271 6.25307 15.9603 4.93829 14.4798C3.64922 13.0282 2.75 11.3345 2.75 9.1371H1.25C1.25 11.8026 2.3605 13.8361 3.81672 15.4758C5.24723 17.0866 7.07077 18.3752 8.49742 19.4999L9.42605 18.3219ZM2.75 9.1371C2.75 6.98623 3.96537 5.18252 5.62436 4.42419C7.23607 3.68748 9.40166 3.88258 11.4596 6.02073L12.5404 4.98053C10.0985 2.44352 7.26409 2.02539 5.00076 3.05996C2.78471 4.07292 1.25 6.42503 1.25 9.1371H2.75ZM8.49742 19.4999C9.00965 19.9037 9.55954 20.3343 10.1168 20.6599C10.6739 20.9854 11.3096 21.25 12 21.25V19.75C11.6904 19.75 11.3261 19.6293 10.8736 19.3648C10.4213 19.1005 9.95208 18.7366 9.42605 18.3219L8.49742 19.4999ZM15.5026 19.4999C16.9292 18.3752 18.7528 17.0866 20.1833 15.4758C21.6395 13.8361 22.75 11.8026 22.75 9.1371H21.25C21.25 11.3345 20.3508 13.0282 19.0617 14.4798C17.7469 15.9603 16.0896 17.1271 14.574 18.3219L15.5026 19.4999ZM22.75 9.1371C22.75 6.42503 21.2153 4.07292 18.9992 3.05996C16.7359 2.02539 13.9015 2.44352 11.4596 4.98053L12.5404 6.02073C14.5983 3.88258 16.7639 3.68748 18.3756 4.42419C20.0346 5.18252 21.25 6.98623 21.25 9.1371H22.75ZM14.574 18.3219C14.0479 18.7366 13.5787 19.1005 13.1264 19.3648C12.6739 19.6293 12.3096 19.75 12 19.75V21.25C12.6904 21.25 13.3261 20.9854 13.8832 20.6599C14.4405 20.3343 14.9903 19.9037 15.5026 19.4999L14.574 18.3219Z"></path>{" "}
      </g>
    </svg>
  ),
  arrow: (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      fill="none"
      viewBox="0 0 24 24"
      strokeWidth={1.5}
      stroke="currentColor"
      {...props}>
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="m8.25 4.5 7.5 7.5-7.5 7.5"
      />
    </svg>
  ),
  add: (props) => (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}>
      <g id="SVGRepo_bgCarrier" strokeWidth={0} />
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <g id="SVGRepo_iconCarrier">
        <g id="Edit / Add_Plus">
          <path
            id="Vector"
            d="M6 12H12M12 12H18M12 12V18M12 12V6"
            stroke="current color"
            strokeWidth={2}
            strokeLinecap="round"
            strokeLinejoin="round"
          />
        </g>
      </g>
    </svg>
  ),
  edit: (props) => (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}>
      <g id="SVGRepo_bgCarrier" strokeWidth={0} />
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <g id="SVGRepo_iconCarrier">
        <path
          d="M11 2H9C4 2 2 4 2 9V15C2 20 4 22 9 22H15C20 22 22 20 22 15V13"
          stroke="#current Color"
          strokeWidth={1.5}
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M16.04 3.02001L8.16 10.9C7.86 11.2 7.56 11.79 7.5 12.22L7.07 15.23C6.91 16.32 7.68 17.08 8.77 16.93L11.78 16.5C12.2 16.44 12.79 16.14 13.1 15.84L20.98 7.96001C22.34 6.60001 22.98 5.02001 20.98 3.02001C18.98 1.02001 17.4 1.66001 16.04 3.02001Z"
          stroke="#current Color"
          strokeWidth={1.5}
          strokeMiterlimit={10}
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M14.91 4.1499C15.58 6.5399 17.45 8.4099 19.85 9.0899"
          stroke="#current Color"
          strokeWidth={1.5}
          strokeMiterlimit={10}
          strokeLinecap="round"
          strokeLinejoin="round"
        />
      </g>
    </svg>
  ),
  delete: (props) => (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}>
      <g id="SVGRepo_bgCarrier" strokeWidth={0} />
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <g id="SVGRepo_iconCarrier">
        <path
          d="M10 12V17"
          stroke="current Color"
          strokeWidth={1.5}
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M14 12V17"
          stroke="current Color"
          strokeWidth={1.5}
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M4 7H20"
          stroke="current Color"
          strokeWidth={1.5}
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M6 10V18C6 19.6569 7.34315 21 9 21H15C16.6569 21 18 19.6569 18 18V10"
          stroke="current Color"
          strokeWidth={1.5}
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M9 5C9 3.89543 9.89543 3 11 3H13C14.1046 3 15 3.89543 15 5V7H9V5Z"
          stroke="current Color"
          strokeWidth={1.5}
          strokeLinecap="round"
          strokeLinejoin="round"
        />
      </g>
    </svg>
  ),
  addImage: (props) => (
    <svg stroke="currentColor" fill="none" viewBox="0 0 48 48" {...props}>
      <path
        d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4H12a4 4 0 01-4-4v-4m32-4l-3.172-3.172a4 4 0 00-5.656 0L28 28M8 32l9.172-9.172a4 4 0 015.656 0L28 28m0 0l4 4m4-24h8m-4-4v8m-12 4h.02"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  ),
  // SUB HEADER MENU ICON --------------------------
  menu: (props) => (
    <svg
      viewBox="0 0 24 24"
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
      fill="white"
      {...props}>
      <g id="SVGRepo_bgCarrier" strokeWidth={0} />
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <g id="SVGRepo_iconCarrier">
        <title>{"Menu"}</title>
        <g
          id="Page-1"
          stroke="none"
          strokeWidth={1}
          fill="none"
          fillRule="evenodd">
          <g id="Menu">
            <rect
              id="Rectangle"
              fillRule="nonzero"
              x={0}
              y={0}
              width={40}
              height={40}
            />
            <line
              x1={5}
              y1={7}
              x2={19}
              y2={7}
              id="Path"
              stroke="white"
              strokeWidth={2}
              strokeLinecap="round"
            />
            <line
              x1={5}
              y1={17}
              x2={19}
              y2={17}
              id="Path"
              stroke="white"
              strokeWidth={2}
              strokeLinecap="round"
            />
            <line
              x1={5}
              y1={12}
              x2={19}
              y2={12}
              id="Path"
              stroke="white"
              strokeWidth={2}
              strokeLinecap="round"
            />
          </g>
        </g>
      </g>
    </svg>
  ),
  // HEADER MENU ICON ------------------------------
  topMenu: () => (
    <svg
      className="w-10"
      viewBox="0 0 24 24"
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
      fill="none">
      <g id="SVGRepo_bgCarrier" strokeWidth={0} />
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <g id="SVGRepo_iconCarrier">
        <title>{"Menu"}</title>
        <g
          id="Page-1"
          stroke="none"
          strokeWidth={1}
          fill="curre"
          fillRule="evenodd">
          <g id="Menu">
            <rect
              id="Rectangle"
              fillRule="nonzero"
              x={0}
              y={0}
              width={40}
              height={40}
            />
            <line
              x1={5}
              y1={7}
              x2={19}
              y2={7}
              id="Path"
              className="stroke-black dark:stroke-white transition-colors duration-300"
              stroke="black"
              strokeWidth={2}
              strokeLinecap="round"
            />
            <line
              x1={5}
              y1={17}
              x2={19}
              y2={17}
              id="Path"
              className="stroke-black dark:stroke-white transition-colors duration-300"
              stroke="black"
              strokeWidth={2}
              strokeLinecap="round"
            />
            <line
              x1={5}
              y1={12}
              x2={19}
              y2={12}
              id="Path"
              className="stroke-black dark:stroke-white transition-colors duration-300"
              stroke="black"
              strokeWidth={2}
              strokeLinecap="round"
            />
          </g>
        </g>
      </g>
    </svg>
  ),

  // SUB HEADER SPECEFIC
  Coffee: ({ className, fill }) => (
    <svg
      width={43}
      height={35}
      viewBox="0 0 37 35"
      fill="none"
      className={className}
      xmlns="http://www.w3.org/2000/svg">
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M30.5058 4.35994C31.168 4.54545 31.6262 5.14771 31.629 5.8349C31.6463 5.87876 31.6465 5.90483 31.6465 5.90483C31.6465 5.90483 31.6392 11.7911 24.6696 17.2096C22.0785 19.2237 19.8016 22.39 18.9383 25.4768C18.7694 26.0734 18.2612 26.5114 17.6467 26.5922C17.0321 26.6722 16.4284 26.3795 16.1116 25.8468C14.3306 22.7494 12.8203 17.6918 16.2136 11.8076C19.521 6.07326 25.5603 3.0745 30.5058 4.35994ZM29.3792 6.31129C25.07 5.77685 20.8436 8.39478 18.2083 12.9592C16.1152 16.5847 16.519 20.7016 17.5454 23.4245C18.3928 20.9018 20.0445 17.8878 23.2561 15.3908C27.3483 12.2094 28.9395 8.06679 29.3792 6.31129Z"
        fill="none"
        className={fill}
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M21.4031 30.4859C20.7209 30.2982 20.2467 29.6787 20.2438 28.9709C20.2262 28.9461 20.2258 28.9319 20.2258 28.9319C20.2258 28.9319 20.2331 23.0456 27.2027 17.6271C29.8182 15.5938 32.1132 12.3868 32.9578 9.27255C33.1178 8.69137 33.6079 8.26158 34.2055 8.17903C34.8022 8.0967 35.3909 8.37738 35.702 8.89344C37.5091 11.9801 39.0879 17.0825 35.6587 23.0291C32.3594 28.7495 26.3418 31.7475 21.4031 30.4859ZM22.4931 28.5254C26.8023 29.0598 31.0287 26.4419 33.664 21.8774C35.7571 18.252 35.3533 14.1351 34.3269 11.4121C33.4795 13.9349 31.8278 16.9489 28.6162 19.4458C24.524 22.6273 22.9328 26.7699 22.4931 28.5254Z"
        fill="none"
        className={fill}
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M12.6943 30.3733C13.374 30.3647 13.9654 29.9061 14.1443 29.2507C14.174 29.2053 14.1818 29.1764 14.1818 29.1764C14.1818 29.1764 15.6982 23.4882 10.3685 16.4514C8.39351 13.8436 7.01694 10.2116 6.97319 7.01391C6.96147 6.38812 6.57947 5.82955 5.99978 5.59205C5.42087 5.35455 4.756 5.48423 4.30834 5.92173C1.79505 8.45846 -0.945573 12.9436 0.802867 19.4803C2.51459 25.8811 7.57944 30.3429 12.6943 30.3733ZM16.2404 26.9538C16.8459 27.3007 17.3435 27.446 18.06 27.3241C17.7576 28.0467 17.3099 28.5811 16.8818 28.9608C16.6865 29.1147 16.417 29.1358 16.2006 29.0139C15.9834 28.8928 15.8607 28.6522 15.8904 28.4053C15.9099 28.0116 16.0045 27.5436 16.2404 26.9538ZM12.0966 28.1968C7.79584 27.5975 4.39195 23.9756 3.02789 18.8842C1.94429 14.8405 3.39974 10.9686 5.09584 8.60375C5.26146 11.26 6.07634 14.5991 8.5326 17.8421C11.6623 21.9741 12.1271 26.3874 12.0966 28.1968ZM13.5857 17.6491C13.3748 17.3217 13.1537 17.0053 12.9248 16.703C7.59432 9.66548 9.11148 3.97798 9.11148 3.97798C9.11148 3.97798 9.12006 3.94594 9.13413 3.8936C9.31303 3.23813 9.89978 2.77563 10.5795 2.7561C13.6099 2.79204 16.6248 4.35999 18.9357 6.96625C18.3756 7.51156 17.867 8.04439 17.4076 8.57173C15.7584 6.59205 13.6193 5.29438 11.1959 4.95688C11.1685 6.63422 11.5662 10.5506 14.1185 14.4053C13.8341 15.4264 13.6638 16.496 13.5857 17.6491Z"
        fill="none"
        className={fill}
      />
    </svg>
  ),
  Off: ({ className, fill, stroke }) => (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}>
      <g id="SVGRepo_bgCarrier" strokeWidth={0} />
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <g id="SVGRepo_iconCarrier">
        <path
          opacity={0.5}
          d="M9.78133 3.89027C10.3452 3.40974 10.6271 3.16948 10.9219 3.02859C11.6037 2.70271 12.3963 2.70271 13.0781 3.02859C13.3729 3.16948 13.6548 3.40974 14.2187 3.89027C14.4431 4.08152 14.5553 4.17715 14.6752 4.25747C14.9499 4.4416 15.2584 4.56939 15.5828 4.63344C15.7244 4.66139 15.8713 4.67312 16.1653 4.69657C16.9038 4.7555 17.273 4.78497 17.5811 4.89378C18.2936 5.14546 18.8541 5.70591 19.1058 6.41844C19.2146 6.72651 19.244 7.09576 19.303 7.83426C19.3264 8.12819 19.3381 8.27515 19.3661 8.41669C19.4301 8.74114 19.5579 9.04965 19.7421 9.32437C19.8224 9.44421 19.918 9.55642 20.1093 9.78084C20.5898 10.3447 20.8301 10.6267 20.971 10.9214C21.2968 11.6032 21.2968 12.3958 20.971 13.0776C20.8301 13.3724 20.5898 13.6543 20.1093 14.2182C19.918 14.4426 19.8224 14.5548 19.7421 14.6747C19.5579 14.9494 19.4301 15.2579 19.3661 15.5824C19.3381 15.7239 19.3264 15.8709 19.303 16.1648C19.244 16.9033 19.2146 17.2725 19.1058 17.5806C18.8541 18.2931 18.2936 18.8536 17.5811 19.1053C17.273 19.2141 16.9038 19.2435 16.1653 19.3025C15.8713 19.3259 15.7244 19.3377 15.5828 19.3656C15.2584 19.4297 14.9499 19.5574 14.6752 19.7416C14.5553 19.8219 14.4431 19.9175 14.2187 20.1088C13.6548 20.5893 13.3729 20.8296 13.0781 20.9705C12.3963 21.2963 11.6037 21.2963 10.9219 20.9705C10.6271 20.8296 10.3452 20.5893 9.78133 20.1088C9.55691 19.9175 9.44469 19.8219 9.32485 19.7416C9.05014 19.5574 8.74163 19.4297 8.41718 19.3656C8.27564 19.3377 8.12868 19.3259 7.83475 19.3025C7.09625 19.2435 6.72699 19.2141 6.41893 19.1053C5.7064 18.8536 5.14594 18.2931 4.89427 17.5806C4.78546 17.2725 4.75599 16.9033 4.69706 16.1648C4.6736 15.8709 4.66188 15.7239 4.63393 15.5824C4.56988 15.2579 4.44209 14.9494 4.25796 14.6747C4.17764 14.5548 4.08201 14.4426 3.89076 14.2182C3.41023 13.6543 3.16997 13.3724 3.02907 13.0776C2.7032 12.3958 2.7032 11.6032 3.02907 10.9214C3.16997 10.6266 3.41023 10.3447 3.89076 9.78084C4.08201 9.55642 4.17764 9.44421 4.25796 9.32437C4.44209 9.04965 4.56988 8.74114 4.63393 8.41669C4.66188 8.27515 4.6736 8.12819 4.69706 7.83426C4.75599 7.09576 4.78546 6.72651 4.89427 6.41844C5.14594 5.70591 5.7064 5.14546 6.41893 4.89378C6.72699 4.78497 7.09625 4.7555 7.83475 4.69657C8.12868 4.67312 8.27564 4.66139 8.41718 4.63344C8.74163 4.56939 9.05014 4.4416 9.32485 4.25747C9.4447 4.17715 9.55691 4.08152 9.78133 3.89027Z"
          stroke="none"
          className={stroke}
          strokeWidth={1.5}
        />
        <path
          d="M9 15L15 9"
          stroke="none"
          className={stroke}
          strokeWidth={1.5}
          strokeLinecap="round"
        />
        <path
          d="M15.5 14.5C15.5 15.0523 15.0523 15.5 14.5 15.5C13.9477 15.5 13.5 15.0523 13.5 14.5C13.5 13.9477 13.9477 13.5 14.5 13.5C15.0523 13.5 15.5 13.9477 15.5 14.5Z"
          stroke="none"
          className={fill}
        />
        <path
          d="M10.5 9.5C10.5 10.0523 10.0523 10.5 9.5 10.5C8.94772 10.5 8.5 10.0523 8.5 9.5C8.5 8.94772 8.94772 8.5 9.5 8.5C10.0523 8.5 10.5 8.94772 10.5 9.5Z"
          stroke="none"
          className={fill}
        />
      </g>
    </svg>
  ),
  Cake: ({ className, stroke }) => (
    <svg
      width={45}
      height={46}
      viewBox="0 0 45 46"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}>
      <path
        d="M1.6665 43.8335H43.3332"
        stroke="none"
        className={stroke}
        strokeWidth={2.5}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M3.979 43.8335V25.0835C3.979 21.6252 7.08317 18.8335 10.9165 18.8335H34.0624C37.8957 18.8335 40.9999 21.6252 40.9999 25.0835V43.8335"
        stroke="none"
        className={stroke}
        strokeWidth={2.5}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M9.0835 18.8334V12.9376C9.0835 10.4376 11.3335 8.41675 14.1252 8.41675H30.8961C33.6669 8.41675 35.9169 10.4376 35.9169 12.9376V18.8334"
        stroke="none"
        className={stroke}
        strokeWidth={2.5}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M4.854 27.125L5.62484 27.1458C7.1665 27.1667 8.39567 28.4167 8.39567 29.9583V30.6458C8.39567 32.1875 9.64567 33.4583 11.2082 33.4583C12.7498 33.4583 14.0207 32.2083 14.0207 30.6458V30C14.0207 28.4583 15.2707 27.1875 16.8332 27.1875C18.3748 27.1875 19.6456 28.4375 19.6456 30V30.6458C19.6456 32.1875 20.8956 33.4583 22.4581 33.4583C23.9998 33.4583 25.2706 32.2083 25.2706 30.6458V30C25.2706 28.4583 26.5206 27.1875 28.0831 27.1875C29.6248 27.1875 30.8956 28.4375 30.8956 30V30.6458C30.8956 32.1875 32.1456 33.4583 33.7081 33.4583C35.2498 33.4583 36.5206 32.2083 36.5206 30.6458V30C36.5206 28.4583 37.7706 27.1875 39.3331 27.1875H40.2706"
        stroke="none"
        className={stroke}
        strokeWidth={2.5}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M14.1665 8.41667V4.25"
        stroke="none"
        className={stroke}
        strokeWidth={2.5}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M30.833 8.41667V4.25"
        stroke="none"
        className={stroke}
        strokeWidth={2.5}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M22.5 8.41675V2.16675"
        stroke="none"
        className={stroke}
        strokeWidth={2.5}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  ),
  WarmDrink: ({ className, fill }) => (
    <svg
      width={34}
      height={44}
      viewBox="0 0 34 44"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}>
      <path
        d="M7.42773 0C6.06275 0 4.85946 0.932387 4.52148 2.25586L4.07422 4H4C3.13355 4 2.3433 4.29325 1.76367 4.78906C1.18405 5.28487 0.815392 5.93804 0.566406 6.625C0.0684347 7.99892 0 9.57785 0 11C2.65169e-05 11.2652 0.105392 11.5195 0.292923 11.7071C0.480454 11.8946 0.734792 12 1 12H2.11914L5.9707 42.0176C5.9719 42.0261 5.9732 42.0345 5.97461 42.043C6.16052 43.2378 7.20443 44 8.36719 44H25.6309C26.7942 44 27.8379 43.2357 28.0234 42.043C28.0248 42.0345 28.0261 42.0261 28.0273 42.0176L31.8809 12H33C33.2652 12 33.5195 11.8946 33.7071 11.7071C33.8946 11.5195 34 11.2652 34 11C34 9.57785 33.9316 7.99892 33.4336 6.625C33.1846 5.93804 32.816 5.28487 32.2363 4.78906C31.6567 4.29325 30.8664 4 30 4H29.9062L29.4746 2.27344V2.27148C29.1414 0.941602 27.9344 0 26.5625 0H7.42773ZM7.42773 2H26.5625C27.0266 2 27.4204 2.30769 27.5332 2.75781L27.8438 4H6.13867L6.45898 2.75195C6.57301 2.30543 6.96672 2 7.42773 2ZM4.91211 5.99609C4.94133 5.99868 4.97066 5.99999 5 6H29C29.0215 6.00004 29.043 5.99939 29.0645 5.99805C29.0846 5.99931 29.1048 5.99996 29.125 6H30C30.4591 6 30.7061 6.11065 30.9375 6.30859C31.1689 6.50654 31.385 6.8386 31.5547 7.30664C31.8021 7.98926 31.83 9.02214 31.8789 10H3.16602C3.09638 9.988 3.02568 9.98342 2.95508 9.98633C2.91513 9.9885 2.87534 9.99306 2.83594 10H2.12109C2.16997 9.02214 2.1979 7.98926 2.44531 7.30664C2.61495 6.8386 2.83109 6.50654 3.0625 6.30859C3.29391 6.11065 3.54095 6 4 6H4.84961C4.87048 5.99935 4.89132 5.99805 4.91211 5.99609ZM4.13672 12H29.8633L28.709 21H23.9844C22.6596 18.0853 20.0955 16.012 16.9922 16.0039C16.946 16.0023 16.9028 16.0048 16.8574 16.0078C13.7862 16.0598 11.2781 18.123 9.98828 21H5.29102L4.13672 12ZM16.9453 17.998C16.9663 17.9991 16.9478 18 16.9746 18C19.3928 18 21.4744 19.6826 22.4512 22.3438C22.5215 22.5359 22.6491 22.7019 22.8168 22.8192C22.9844 22.9366 23.184 22.9997 23.3887 23H23.4902H28.4512L27.8105 28H23.5273H23.3887C23.1829 27.9998 22.9821 28.0631 22.8136 28.1813C22.6451 28.2994 22.5172 28.4666 22.4473 28.6602C22.2275 29.2702 21.9767 29.8288 21.6719 30.2969C20.5385 32.0342 18.8847 32.976 17.0879 32.998H17.0215C15.246 32.998 13.5923 32.0964 12.4316 30.3984V30.3965C12.0919 29.9003 11.8123 29.3066 11.5742 28.6562C11.5037 28.4638 11.3758 28.2976 11.2078 28.1802C11.0398 28.0628 10.8397 27.9999 10.6348 28H10.5215H6.18945L5.54883 23H10.4844H10.5898C10.7953 22.9998 10.9957 22.9363 11.1637 22.8182C11.3318 22.7001 11.4595 22.533 11.5293 22.3398C12.4777 19.7084 14.5119 18.0268 16.918 18C16.9271 17.9995 16.9362 17.9988 16.9453 17.998ZM16.9395 20C15.7051 20.0135 14.6652 20.7779 14.002 21.7949C13.3388 22.8119 12.9786 24.12 12.9941 25.543C13.0097 26.966 13.3988 28.2673 14.084 29.2695C14.7692 30.2718 15.8262 31.0135 17.0605 31C18.2949 30.9865 19.3348 30.2221 19.998 29.2051C20.6612 28.1881 21.0214 26.88 21.0059 25.457C20.9903 24.034 20.6012 22.7328 19.916 21.7305C19.2308 20.7282 18.1738 19.9865 16.9395 20ZM16.9609 22C17.3861 21.9954 17.8436 22.2449 18.2637 22.8594C18.6837 23.4738 18.9943 24.4164 19.0059 25.4785C19.0174 26.5406 18.7288 27.4898 18.3223 28.1133C17.9157 28.7367 17.4642 28.9954 17.0391 29C16.6139 29.0046 16.1564 28.7551 15.7363 28.1406C15.3163 27.5262 15.0057 26.5836 14.9941 25.5215C14.9826 24.4594 15.2712 23.5102 15.6777 22.8867C16.0843 22.2633 16.5358 22.0046 16.9609 22ZM6.44531 30H10.0215C10.2458 30.5199 10.4542 31.0475 10.7812 31.5254C12.2646 33.6954 14.553 34.998 17.0215 34.998H17.0918C17.0951 34.9981 17.0983 34.9981 17.1016 34.998C19.6015 34.9713 21.8948 33.6147 23.3457 31.3906H23.3477C23.6356 30.9487 23.8146 30.4711 24.0137 30H27.5547L26.0488 41.7363C26.0424 41.7776 25.8275 42 25.6309 42H8.36719C8.17395 42 7.95727 41.7755 7.95117 41.7363L6.44531 30Z"
        fill="none"
        className={fill}
      />
    </svg>
  ),
  ColdDrink: ({ className }) => (
    <svg
      fill="none"
      height="200px"
      width="200px"
      id="Capa_1"
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
      viewBox="0 0 420.339 420.339"
      xmlSpace="preserve"
      className={className}>
      <g id="SVGRepo_bgCarrier" strokeWidth={0} />
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <g id="SVGRepo_iconCarrier">
        <g>
          <g>
            <polygon points="180.68,280.563 184.549,282.767 184.817,282.298 " />
            <path d="M360.251,48.376L300.061,8.79c-3.229-2.123-7.57-1.228-9.695,2.002c-2.124,3.23-1.227,7.57,2.002,9.695l60.19,39.586 c1.185,0.779,2.521,1.152,3.84,1.152c2.277,0,4.511-1.109,5.855-3.154C364.377,54.841,363.481,50.501,360.251,48.376z" />
            <path d="M262.414,98.863h-19.905l21.112-82.489c0.959-3.745-1.301-7.559-5.046-8.517c-3.748-0.961-7.558,1.301-8.517,5.045 l-22,85.961H63.868c-0.726,0-1.456,0.089-2.14,0.332c-3.423,1.214-5.202,4.51-4.715,7.719l46.684,307.476 c0.52,3.421,3.461,5.949,6.921,5.949h105.037c3.46,0,6.401-2.528,6.921-5.949l46.684-307.476c0.486-3.206-1.287-6.493-4.7-7.715 C263.875,98.954,263.142,98.863,262.414,98.863z M72.076,112.863h152.399l-11.778,46.021h-22.971l-11.564-34.26 c-0.145-0.431-0.315-0.857-0.556-1.243c-1.867-2.995-5.31-4.057-8.284-3.053l-63.914,21.575c-3.262,1.101-5.014,4.638-3.913,7.9 l3.066,9.082H79.064L72.076,112.863z M161.22,288.657l34.84,39.529l-39.53,34.839l-34.84-39.529L161.22,288.657z M180.68,280.563 l-41.916-23.874l26.078-45.785l45.786,26.079l-25.812,45.315l-0.268,0.469L180.68,280.563z M179.676,172.885l4.344,12.87 l-49.924,16.853l-10.032-29.723H179.676z M119.337,158.885l-2.093-6.201l49.924-16.853l7.781,23.054H119.337z M209.638,406.339 h-84.521h-8.481L81.19,172.885h28.098l13.815,40.931c0.125,0.37,0.269,0.737,0.466,1.074c1.388,2.375,3.753,3.589,6.134,3.589 c0.755,0,1.511-0.122,2.239-0.368l15.702-5.3l-24.51,43.031c-1.913,3.359-0.741,7.634,2.618,9.547l24.72,14.08l-43.29,38.153 c-1.394,1.228-2.241,2.958-2.358,4.811c-0.116,1.853,0.507,3.677,1.735,5.069l7.96,9.031l36.137,41.001 c1.383,1.569,3.313,2.371,5.254,2.371c1.644,0,3.295-0.576,4.625-1.748l50.033-44.096c2.346-2.067,3.145-5.56,1.514-8.665 c-0.214-0.407-0.501-0.771-0.804-1.116l-22.235-25.229c1.718-0.478,3.256-1.603,4.207-3.273l33.008-57.95 c1.575-2.763,1.159-6.41-1.523-8.806c-0.29-0.259-0.621-0.471-0.959-0.663l-45.47-25.898l17.55-5.924 c3.262-1.101,5.014-4.639,3.913-7.901l-1.941-5.75l-3.375-10h50.632L209.638,406.339z M247.209,158.885h-20.062l11.778-46.021 h15.271L247.209,158.885z" />
            <path d="M276.92,0c-3.866,0-7,3.134-7,7v10.034c0,3.866,3.134,7,7,7c3.866,0,7-3.134,7-7V7C283.92,3.134,280.786,0,276.92,0z" />
            <path d="M189.801,260.393l8.991-15.283c1.961-3.332,0.849-7.622-2.483-9.582c-3.333-1.963-7.622-0.849-9.583,2.483l-8.991,15.283 c-1.961,3.332-0.849,7.623,2.483,9.583c1.115,0.656,2.337,0.968,3.543,0.968C186.16,263.845,188.496,262.61,189.801,260.393z" />
            <path d="M160.069,334.147l-11.623-13.392c-2.533-2.919-6.956-3.232-9.874-0.698c-2.92,2.534-3.232,6.955-0.698,9.875 l11.623,13.392c1.384,1.595,3.331,2.412,5.289,2.412c1.627,0,3.261-0.563,4.585-1.714 C162.291,341.487,162.604,337.066,160.069,334.147z" />
          </g>
        </g>
      </g>
    </svg>
  ),
  Food: ({ className, fill }) => (
    <svg
      viewBox="-4.5 0 133 133"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}>
      <g id="SVGRepo_bgCarrier" strokeWidth={0} />
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <g id="SVGRepo_iconCarrier">
        <g clipPath="url(#clip0)">
          <path
            d="M91.2031 91.2616C89.1028 91.5024 88.2017 93.1774 87.1522 94.5774C82.882 100.272 78.5167 101.797 71.6462 99.899C66.3581 98.4367 64.9016 99.1252 62.3432 104.092C61.4362 106.021 60.3567 107.864 59.118 109.599C58.2213 110.749 57.0771 111.682 55.7708 112.329C54.4645 112.976 53.0297 113.32 51.572 113.337C50.1142 113.355 48.671 113.043 47.35 112.426C46.029 111.81 44.8639 110.904 43.9407 109.776C43.6588 109.426 43.3574 109.093 43.0377 108.777C41.3424 107.195 39.6013 107.522 38.9817 109.791C38.4566 111.714 38.4378 113.776 38.2068 115.778C37.8596 118.784 37.5697 121.799 37.1569 124.796C36.9962 125.793 36.6862 126.761 36.2378 127.666C33.9609 132.468 28.3535 133.018 25.5306 128.5C24.147 126.286 23.5295 123.557 22.7675 121.002C22.161 118.967 22.0282 116.792 21.383 114.777C20.5888 112.299 19.4096 111.927 17.0139 112.999C15.6213 113.717 14.1832 114.344 12.709 114.874C7.50486 116.418 2.67415 113.627 1.21117 108.41C0.524642 105.962 0.178799 103.847 2.43988 101.741C3.9278 100.354 4.67142 98.1485 5.68087 96.2714C16.2479 76.5813 27.8883 57.5338 40.32 38.9732C47.0028 28.9969 53.8043 19.0947 60.5908 9.18599C61.8576 7.33579 63.3687 5.65417 64.6551 3.81576C66.2609 1.51858 68.415 0.787409 71.0988 1.16349C76.5615 1.92944 81.4105 4.27332 85.9235 7.24391C103.993 19.1374 115.797 35.4474 120.359 56.7489C121.318 61.2264 122.693 65.6362 122.023 70.3664C121.798 71.9534 122.575 73.6599 122.717 75.327C123.044 78.0232 122.519 80.7543 121.217 83.1374C119.929 85.369 118.209 86.5917 115.43 86.213C112.16 85.768 111.492 86.3213 110.947 89.5859C110.488 92.3504 110.245 95.151 109.831 97.9214C109.642 99.1403 109.356 100.343 108.978 101.516C107.791 105.261 105.737 106.909 102.239 107.02C98.3736 107.144 95.357 105.248 94.1665 101.68C93.6414 100.105 93.6 98.3776 93.2266 96.7453C92.7881 94.8458 93.1071 92.6405 91.2031 91.2616ZM82.0734 53.4015C82.0262 53.2998 82.0085 53.1494 81.9284 53.1035C81.4657 52.8028 80.9773 52.5433 80.4693 52.3277C76.085 50.746 73.5312 47.5752 72.4036 43.139C72.0177 41.6222 71.6678 40.0929 71.2038 38.6004C70.7024 36.6548 70.0375 34.7547 69.2177 32.9204C67.2743 29.1045 63.1791 27.5595 57.8969 28.2151C54.1098 28.6851 53.3229 31.7548 51.6617 34.5481C57.1513 35.2596 59.565 38.5899 60.1524 43.2085C60.5809 46.2267 60.2831 49.3036 59.2846 52.1838C58.286 55.0641 56.6154 57.6646 54.4108 59.7698C49.3307 64.8427 42.3254 65.5011 36.1512 61.8361C35.5021 61.4508 34.8827 61.0144 33.9534 60.4092L9.06097 100.544L9.49553 101.177L26.7132 95.6551C26.7867 94.8891 26.836 94.5603 26.8446 94.2302C26.8577 93.7819 26.8006 93.3284 26.8446 92.884C27.5849 85.1734 30.5022 78.8693 37.8709 75.245C45.382 71.5511 57.0079 72.4378 61.6305 82.252C61.8611 82.6629 62.1186 83.0573 62.4015 83.4334C63.751 82.974 65.0206 82.5664 66.2741 82.1116C67.5979 81.6318 68.9053 81.1074 70.1852 80.6151C66.9284 70.2023 71.7538 59.405 82.0734 53.4015ZM29.8295 126.598L30.7717 126.381C32.3692 123.952 32.3574 121.124 32.6305 118.367C32.9389 115.247 32.9704 112.072 33.59 109.016C34.5167 104.442 38.3812 102.003 42.5627 103.102C44.3683 103.576 46.1899 104.07 47.5019 105.696C48.7023 107.182 50.4744 108.073 52.2629 107.033C53.4753 106.258 54.4327 105.142 55.0152 103.825C56.6491 100.235 59.3358 97.2264 62.7188 95.1976C63.9155 94.3628 65.2849 93.8095 66.7256 93.5778C68.1656 93.3461 69.6404 93.4426 71.0384 93.8594C72.1155 94.1691 73.1702 94.5925 74.2669 94.792C77.1174 95.3105 79.6082 94.8117 81.3226 92.1489C82.0203 91.1303 82.7921 90.1648 83.6329 89.261C86.0574 86.5077 88.9203 84.4895 92.7645 85.6217C96.4512 86.7072 97.7783 89.836 98.3906 93.3225C98.7011 95.0887 98.9242 96.8713 99.2478 98.6355C99.4775 99.8885 99.8746 101.149 101.439 101.199C103.003 101.25 102.962 99.8169 103.312 98.818C103.478 98.2818 103.586 97.7291 103.632 97.1699C103.96 94.3877 104.259 91.6022 104.584 88.8206C105.351 82.277 109.668 79.7993 114.633 80.5508C114.817 80.5784 115.033 80.3946 115.618 80.1471C115.854 78.3494 116.118 76.3509 116.445 73.8614C111.136 75.3211 106.442 76.4211 101.874 77.9012C80.1182 84.9423 58.4325 92.1995 36.6454 99.1409C26.9382 102.234 17.0525 104.768 7.24547 107.549C6.57269 107.74 5.88443 107.877 5.08895 108.066C7.38941 111.352 8.614 111.777 11.6712 110.442C12.6938 109.995 13.62 109.333 14.6334 108.861C15.6101 108.329 16.6538 107.929 17.7366 107.674C21.5473 107.052 24.6831 108.926 26.0155 112.932C26.9646 115.786 27.325 118.83 28.0896 121.752C28.5222 123.404 29.2413 124.984 29.832 126.598H29.8295ZM104.481 69.6352C104.405 68.6343 104.273 67.6387 104.087 66.6528C100.3 51.9667 93.8317 38.6529 83.4846 27.4414C78.2188 21.7365 71.5563 18.1726 64.1289 15.7487L57.9136 24.7058C59.2597 24.7058 59.9331 24.757 60.5966 24.6979C67.4929 24.0822 72.3859 28.5604 74.3556 34.7103C74.9699 36.6261 75.4891 38.5742 76.0062 40.5189C76.9947 44.2331 78.9545 46.9923 82.9129 47.9952C83.9873 48.3201 85.0184 48.7752 85.9832 49.3505C90.1313 51.5938 92.6726 55.0056 92.8991 59.7785C92.9233 62.4656 92.8111 65.1521 92.563 67.8277C92.4678 69.5598 92.3169 71.2892 92.1758 73.23L104.481 69.6352ZM115.878 66.6988C115.786 65.5451 115.761 64.8775 115.675 64.2185C113.018 43.9062 104.294 27.0134 87.46 14.8463C83.9158 12.3048 80.1359 10.1092 76.1716 8.28944C70.94 5.87086 70.3821 6.24818 67.3813 11.2258C67.2914 11.3748 67.3701 11.6262 67.3701 11.8218C67.5618 11.9508 67.7633 12.0638 67.9733 12.1592C68.6054 12.387 69.2545 12.5681 69.8833 12.8044C77.3484 15.6069 83.9191 19.6815 89.259 25.7395C97.9516 35.6356 104.468 47.2473 108.387 59.8224C109.229 62.4622 110.071 65.102 110.963 67.8999L115.878 66.6988ZM56.5663 85.4458C55.0193 80.5941 51.0162 78.3921 45.797 79.1712C38.3679 80.2797 33.4375 85.8035 33.433 93.2739L56.5663 85.4458ZM37.2697 55.719C42.1319 59.5488 46.8831 59.2436 50.0138 55.0811C51.944 52.5676 52.9744 49.4788 52.9398 46.3098C52.9037 42.4991 51.4676 41.0354 47.25 40.5799L37.2697 55.719ZM86.0266 75.2712C86.467 72.1339 86.8844 69.3642 87.2381 66.5859C87.5985 63.7525 88.5699 60.9094 87.3931 58.0431C79.7612 59.5297 74.4284 70.5456 77.7153 78.0272L86.0266 75.2712Z"
            fill="none"
            className={fill}
          />
        </g>
        <defs>
          <clipPath id="clip0">
            <rect
              width={123.392}
              height={131.268}
              fill="none"
              className={fill}
              transform="translate(0.0473633 0.77832)"
            />
          </clipPath>
        </defs>
      </g>
    </svg>
  ),
};
